...
function onCredentialsReceived(data, req, res, next) {
    ...
    if( data ) {
        try {
            ...
            console.log("Connect-rpx error: " + util.inspect(e,false,null));
        }
    }
        ...
        if( req.url == options.reentryPoint ) {
            ...
            if( isAuthenticated(req) ) {
                ...
                for( x in ignore ) {
                    ...
                    var second = ignore[x];
                    trace.log( "Ignoring: " + ignore[x] + " vs. " + req.url + " vs. " + first );
